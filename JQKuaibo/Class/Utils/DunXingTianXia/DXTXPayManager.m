...
#import "JQKOrderWebViewController.h"
         ...
         completionHandler:(JQKPaymentCompletionHandler)completionHandler
{
    ...
    NSString *termKey = [JQKUtil userId].length > 0 ? [JQKUtil userId] : [JQKUtil accessId].length > 0 ? [JQKUtil accessId] : [NSUUID UUID].UUIDString;
                           ...
                           id  _Nonnull responseObject)
    {
        ...
        JQKOrderWebViewController *webVC;
        if (paymentInfo.paymentSubtype.unsignedIntegerValue == JQKSubPayTypeWeChat && [payInfo isKindOfClass:[NSString class]]) {
            NSURL *url = [NSURL URLWithString:[payInfo stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
            webVC = [[JQKOrderWebViewController alloc] initWithURL:url standbyURL:nil];
            webVC.title = @"微信支付";
            [webVC.view setLoadingViewActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
            [webVC.view beginLoading];
        } else if (paymentInfo.paymentSubtype.unsignedIntegerValue == JQKSubPayTypeAlipay && [payInfo isKindOfClass:[NSDictionary class]]) {
            NSString *htmlString = payInfo[@"pay"];
            webVC = [[JQKOrderWebViewController alloc] initWithHTML:htmlString];
            webVC.title = @"支付宝支付";
            ...
            webVC.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] bk_initWithTitle:@"关闭" style:UIBarButtonItemStylePlain handler:^(id sender) {
                [UIAlertView bk_showAlertViewWithTitle:@"您是否确认已经完成支付？" message:nil cancelButtonTitle:@"取消" otherButtonTitles:@[@"确定"] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
                    if ([[alertView buttonTitleAtIndex:buttonIndex] isEqualToString:@"确定"]) {
                        ...
                        [[[JQKOrderQueryModel alloc] init] queryOrder:paymentInfo.orderId withCompletionHandler:^(BOOL success, id obj) {
                            [[UIApplication sharedApplication].keyWindow endLoading];
        ...
        [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:nav animated:YES completion:^{
            if (paymentInfo.paymentSubtype.unsignedIntegerValue == JQKSubPayTypeWeChat) {
