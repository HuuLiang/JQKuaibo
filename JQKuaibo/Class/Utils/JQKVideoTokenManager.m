...
@implementation JQKVideoTokenManager

+ (instancetype)sharedManager {
    static JQKVideoTokenManager *_sharedManager;
...
- (void)requestTokenWithCompletionHandler:(JQKVideoTokenCompletionHandler)completionHandler {
    ...
    if (![JQKUtil userId]) {
        ...
        [self httpRequestTokenWithUserId:[JQKUtil userId] completionHandler:completionHandler];
    }
    
}

- (void)httpRequestTokenWithUserId:(NSString *)userId completionHandler:(JQKVideoTokenCompletionHandler)completionHandler {
    if (userId == nil) {
        QBSafelyCallBlock(completionHandler, NO, nil, nil);
        return ;
    }
    ...
    NSString *dataString = [NSString stringWithFormat:@"uid=%@&channelNo=%@&appId=%@", userId, JQK_CHANNEL_NO, JQK_REST_APP_ID];
